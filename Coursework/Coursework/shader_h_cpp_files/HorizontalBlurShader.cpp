...
{
	initShader(L"PP_general_vs.cso", L"horizontalBlur_ps.cso");
}
...
void HorizontalBlurShader::setShaderParameters(ID3D11DeviceContext* deviceContext, const XMMATRIX &worldMatrix, const XMMATRIX &viewMatrix, const XMMATRIX &projectionMatrix, ID3D11ShaderResourceView* texture, XMFLOAT2 screenSize, bool hasSnorkel, int blurStrength)
{
	...
	widthPtr->screenSize.x = screenSize.x;
	widthPtr->screenSize.y = screenSize.y;
	widthPtr->hasSnorkel = hasSnorkel;
	widthPtr->blurSTR = blurStrength;
