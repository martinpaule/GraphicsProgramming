...
MotionBlurShader::MotionBlurShader(ID3D11Device* device, HWND hwnd, XMINT2 screenMidPos) : BaseShader(device, hwnd)
{
	initShader( L"PP_general_vs.cso", L"MotionBlurMain_ps.cso");
...
void MotionBlurShader::setShaderParameters(ID3D11DeviceContext* deviceContext, const XMMATRIX& worldMatrix, const XMMATRIX& viewMatrix, const XMMATRIX& projectionMatrix, ID3D11ShaderResourceView* texture, float width, int mouseX, int mouseY,bool isLooking)
{

	XMFLOAT2 blurDir = XMFLOAT2(0,0);
	
	//calculate blur direction
	if (isLooking) {
		blurDir.x = mouseX - midX;
		blurDir.y = mouseY - midY;
	}
	...
	widthPtr->blurDirection.x = blurDir.x;																		
	widthPtr->blurDirection.y = blurDir.y;
	widthPtr->padding = 0.0f;																				
