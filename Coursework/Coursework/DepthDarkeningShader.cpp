...
DepthDarkeningShader::DepthDarkeningShader(ID3D11Device* device, HWND hwnd) : BaseShader(device, hwnd)
{
	initShader(L"PP_general_vs.cso", L"depthDarkening_ps.cso");
}
...
void DepthDarkeningShader::initShader(const wchar_t* vsFilename, const wchar_t* psFilename)
{
...
void DepthDarkeningShader::setShaderParameters(ID3D11DeviceContext* deviceContext, const XMMATRIX& worldMatrix, const XMMATRIX& viewMatrix, const XMMATRIX& projectionMatrix, ID3D11ShaderResourceView* texture, float cameraDepth)
{
